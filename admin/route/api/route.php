...
//    Route::get('product/code/:id', 'store.StoreProductController/code')->name('productCode');//产品分享二维码 推广员
    Route::get('product/getGoodList', 'store.StoreProductController/getGoodList')->name('getGoodList');//获取推荐商品
    ...
    //用户类  用户
    Route::post('user/editPwd', 'user.UserController/editPwd')->name('editPwd');// 修改密码
    Route::get('userCenter', 'user.UserController/userCenter')->name('userCenter');//个人中心
    ...
    Route::post('address/tag_list', 'user.UserController/tag_list')->name('tagList');//地址标签列表
    //用户类 收藏
    Route::get('collect/user', 'user.UserController/collect_user')->name('collectUser');//收藏产品列表
    Route::get('collect/foot', 'user.UserController/foot_user')->name('footUser');//足记列表
...
//    Route::post('sign/integral', 'user.UserController/sign_integral')->name('signIntegral');//签到
    ...
    //发票管理类
    Route::post('invoice/list', 'invoice.InvoiceController/list')->name('invoiceList'); //发票管理


    //优惠券类
    Route::post('coupon/receive', 'store.StoreCouponsController/receive')->name('couponReceive'); //领取优惠券
    Route::post('coupon/exchange', 'store.StoreCouponsController/exchange')->name('couponExchange'); //兑换优惠券
//    Route::post('coupon/receive/batch', 'store.StoreCouponsController/receive_batch')->name('couponReceiveBatch'); //批量领取优惠券
    Route::get('coupon/couponsList', 'store.StoreCouponsController/lst')->name('couponsList'); //可领取优惠券列表
    Route::get('coupon/user/:types', 'store.StoreCouponsController/user')->name('couponsUser');//用户已领取优惠券
    Route::get('coupon/order', 'store.StoreCouponsController/order')->name('couponsOrder');//优惠券 订单列表
    //购物车类
    Route::get('cart/list', 'store.StoreCartController/lst')->name('cartList'); //购物车列表
    Route::post('cart/add', 'store.StoreCartController/add')->name('cartAdd'); //购物车新增
    Route::post('cart/del', 'store.StoreCartController/del')->name('cartDel'); //购物车删除
    Route::post('cart/addNum', 'store.StoreCartController/addNum')->name('cartAddNum'); //购物车 修改减产品数量
    Route::get('cart/count', 'store.StoreCartController/count')->name('cartCount'); //购物车 获取数量
    //订单类
    Route::post('order/count', 'order.StoreOrderController/count')->name('orderCount'); //获取订单数量
    ...
    Route::post('order/refund/verify', 'order.StoreOrderController/refund_verify')->name('orderRefundVerify'); //提交订单退款申请
    ...
    Route::post('order/paySuccess', 'order.StoreOrderController/paySuccess')->name('paySuccess'); //支付成功
...
//    Route::get('spread/banner', 'user.UserBillController/spread_banner')->name('spreadBanner');//推广分销二维码海报生成
    Route::get('integral/list', 'user.UserBillController/integral_list')->name('integralList');//积分明细
    //提现类
...
//    Route::post('recharge/wechat', 'user.UserRechargeController/wechat')->name('rechargeWechat');//公众号充值
    //会员等级类
    Route::post('user/level/rechargeMenu', 'user.UserLevelController/rechargeMenu')->name('rechargeMenu');//充值套餐
    Route::post('user/level/recharge', 'user.UserLevelController/recharge')->name('userLevelRecharge');//充值会员
    Route::get('user/user_menu', 'PublicController/user_menu')->name('menuUser');//个人中心菜单
...
Route::group(function () {
    ...
    //发现商品类
    Route::get('discovery', 'store.DiscoveryProductController/index')->name('discovery');//发现商品
    // 首页活动商品
    Route::get('activeProduct/:id', 'store.ActivityProductController/index')->name('activeProduct');// 首页活动商品
    //产品类
    Route::get('product/category', 'store.CategoryController/category')->name('category');//获取商品分类
    Route::get('product/detail/:id/[:type]', 'store.StoreProductController/detail')->name('detail');//产品详情
    Route::get('product/groom/list/:type', 'store.StoreProductController/groom_list')->name('groomList');//获取首页推荐不同类型产品的轮播图和产品
    Route::get('product/list', 'store.StoreProductController/lst')->name('products');//产品列表
    Route::get('product/hot', 'store.StoreProductController/product_hot')->name('productHot');//为你推荐
    Route::get('product/reply/list/:id', 'store.StoreProductController/reply_list')->name('replyList');//产品评价列表
    Route::get('product/reply/config/:id', 'store.StoreProductController/reply_config')->name('replyConfig');//产品评价数量和好评度
...
//通过 订单id 获取 配送员位置
Route::post('delivery/getLocation', 'delivery.LocationController/getLocation')->name('getLocation')
    ->middleware(\app\http\middleware\AllowOriginMiddleware::class);
