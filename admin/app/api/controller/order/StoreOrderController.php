...
class StoreOrderController
{
    ...
    {
        $uid = $request->uid();
        $data['order_count'] = StoreOrder::getOrderCount($uid);
    ...
    {
        list($cartId) = UtilService::postMore([
            ['cartId',''],//购物车id
        ], $request, true);
        $cartId = explode(',', $cartId);
        if (!is_array($cartId) || !$cartId) {
        ...
        $data['discount'] = 0;
        $vipId = UserLevel::getUserLevel($uid); //vip信息
        $user = $request->user(); //用户信息
        if(isset($user['pwd'])) unset($user['pwd']);
        $user['vip'] = $vipId !== false ? true : false;
        if($user['vip']){
            $data['discount'] = UserLevel::getUserLevelInfo($vipId,'discount');
        }
        ...
        $data['productInfo'] = $productInfo = array_column($cartGroup['valid'],'productInfo');
        $data['productTotalNum'] = 0;
        foreach ($cartInfo as &$k) {
            $data['productTotalNum'] += $k['cart_num'];
        }
//        $cate_ids = array_column($productInfo,'cate_id');
//        $product_ids = array_column($productInfo,'id');
...
//        $product_ids = array_unique($product_ids);
        ...
        $data['usableCoupon'] = isset($usableCoupons) ? $usableCoupons : [];
        $data['orderKey'] = StoreOrder::cacheOrderInfo($uid, $cartInfo, $priceGroup, $other); //缓存订单信息
        $data['usableCouponCount'] = count($data['usableCoupon']);//可用优惠券个数
        $data['addressInfo'] = UserAddress::getUserValidAddressList($uid,0); //地址信息
        $data['totalPrice'] = $priceGroup['totalPrice']; //获取订单总价
...
//        $priceGroup = StoreOrder::getOrderPriceGroup($cartInfo);
        ...
        list($addressId, $couponId, $payType, $useIntegral, $mark, $combinationId, $pinkId, $seckill_id, $bargainId,$shipping_type) = UtilService::postMore([
            'addressId', 'couponId', ['payType', 'yue'], ['useIntegral',0], 'mark', ['combinationId', 0], ['pinkId', 0], ['seckill_id', 0], ['bargainId', 0],
            ['shipping_type',1],
        ], $request, true);
    ...
    {
        list($orderKey, $addressId, $couponId, $payType, $useIntegral, $mark, $shipping_type,$take_time) = UtilService::postMore([
            ['orderKey',''], ['addressId',0], ['couponId',0], ['payType',''], ['useIntegral',0], ['mark',''], ['shipping_type',1],['take_time','']
        ], $request, true);
        $uid = $request->uid();
        if (!$orderKey) return app('json')->fail('参数错误!');

        //查找是否已生成订单
        $order = StoreOrder::where(['unique'=>$orderKey,'uid' => $uid, 'is_del' => 0])->find();

        if ($order) {
            if ($order['paid'])
                return app('json')->fail('该订单已支付!');
            if ($order['cart_id']) {
                StoreCart::where('id', 'IN', $order['cart_id'])->update(['is_del' => 1]);
            }
            ...
            switch ($order['pay_type']) {
                case 'weixin_mp_pay':
                    ...
                    return app('json')->status('wechat_mp_pay', ['jsConfig' => $jsConfig, 'order_id' => $order['order_id']]);
                    break;
                case 'weixinh5':
                    try {
                    ...
                    break;
                case 'alipay':
                    if (StoreOrder::aliPay($order['order_id'], $uid))
                        return app('json')->status('success', '支付宝支付成功');
                    else {
                    ...
                    break;
                case 'yue':
                    if (StoreOrder::yuePay($order['order_id'], $uid))
            ...
            $order = StoreOrder::cacheKeyCreateOrder($uid, $orderKey, $addressId, $payType, (int)$useIntegral, $couponId, $mark, false, $shipping_type,strtotime($take_time));
                ...
                switch ($payType) {
                    case "weixin_mp_pay":
                        ...
                        if (bcsub((float)$orderInfo['pay_price'], 0, 2) <= 0) {
                            //创建订单jspay支付
                            $payPriceStatus = StoreOrder::jsPayPrice($orderId, $uid, 0);
                            ...
                            return app('json')->status('weixin_mp_pay', '订单创建成功', $info);
                        }
                        break;
                    case "weixinh5":
                        $orderInfo = StoreOrder::where('order_id', $orderId)->find();
                        ...
                        break;
                    case 'alipay':
                        return app('json')->status('alipay', '支付宝支付成功', $info);
                        break;
                    case 'yue':
                        if (StoreOrder::yuePay($order['order_id'], $uid))
                            return app('json')->status('success', '余额支付成功');
     ...
     * @return mixed
     */
    public static function paySuccess(Request $request)
    {
        list($order_id) = UtilService::postMore([['order_id','']
        ], $request, true);

        if (StoreOrder::be(['order_id' => $order_id, 'paid' => 1])) return app('json')->successful();
        $res = StoreOrder::paySuccess($order_id);
    ...
    public function detail(Request $request, $uni)
    {
        ...
        $order = StoreOrder::getUserOrderDetail($request->uid(),$uni)->hidden(['cart_id', 'freight_price', 'refund_reason', 'is_mer_check', 'cost', 'bargain_id', 'is_channel', 'is_remind', 'is_system_del', 'order_status']);
        if(!$order) return app('json')->fail('订单不存在');
        $order = $order->toArray();
        ...
        $order['_add_time'] = date('Y-m-d H:i:s',$order['add_time']);
    ...
    public function refund_verify(Request $request)
    {
        ...
        if (!$order_id || $data['text'] == '') return app('json')->fail('参数错误!');
        $res = StoreOrder::orderApplyRefund($order_id, $request->uid(), $data['text'], $data['refund_reason_wap_explain'], $data['refund_reason_wap_img']);
        if ($res)
    ...
    {
        list($order_id) = UtilService::postMore([['order_id',0]],$request, true);
        if(!$order_id) return app('json')->fail('参数错误');
        if (StoreOrder::cancelOrder($order_id, $request->uid()))
