...
class PublicController
{
    ...
    public function index(Request $request)
    {
        ...
        $menus = GroupDataService::getData('routine_home_menus') ?: [];//TODO 首页顶部分类
        $button = GroupDataService::getData('routine_home_button') ?: [];//TODO 首页按钮
        ...
        $site_name = sys_config('site_name');//TODO 网站名称
...
//        $info['fastInfo'] = $routine_index_page[0]['fast_info'] ?? '';//sys_config('fast_info');//TODO 快速选择简介
        ...
        $logoUrl = sys_config('routine_index_logo');//TODO 商城logo
...
//        $couponList = StoreCouponIssue::getIssueCouponList($request->uid(), 3);//TODO 优惠券
//        $subscribe = WechatUser::where('uid', $request->uid() ?? 0)->value('subscribe') ? true : false;
//        $newGoodsBananr = sys_config('new_goods_bananr');
        return app('json')->successful(compact('banner', 'menus', 'roll', 'info', 'activity', 'benefit', 'likeInfo', 'logoUrl', 'site_name', 'button'));
    }
    ...
    public function search()
    {
        ...
        return app('json')->successful($routineHotSearch);
    }
