...
class StoreProductRelation extends BaseModel
{
     ...
     * @return int|string
     */
    public static function getUserFootCount($uid = 0){
        $count = self::where('uid',$uid)->where('type','foot')->count();
        return $count;
    }
     ...
     * @return array
     * */
    public static function getUserFootProduct($uid,$page,$limit)
    {
        if(!$limit) return [];
        if($page){
            $list = self::where('A.uid',$uid)
                ->field('B.id pid,A.category,B.store_name,B.price,B.sales,B.image,B.is_del,B.is_show')
                ->alias('A')
                ->where('A.type','foot')/*->where('A.category','product')*/
                ->order('A.add_time DESC')
                ->join('store_product B','A.product_id = B.id')
                ->page($page, $limit)
                ->select();
            ...
            $list = self::where('A.uid',$uid)
                ->field('B.id pid,A.category,B.store_name,B.price,B.sales,B.image,B.is_del,B.is_show')->alias('A')
                ->where('A.type','foot')/*->where('A.category','product')*/
                ->order('A.add_time DESC')->join('store_product B','A.product_id = B.id')
                ->select();
        }
     ...
     * @return array
     * */
    public static function setFootProduct($productId,$uid,$type = '')
    {
        $data = ['uid'=>$uid,'product_id'=>$productId,'type'=>$type,'category'=>'product'];
        if(self::be($data)) return true;
        $data['add_time'] = time();
        self::create($data);
    }
