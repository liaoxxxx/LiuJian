...
class StoreCart extends BaseModel
{
     ...
     */
    public static function changeUserCartNum($cartId,$is_add,$uid)
    {
        ...
        if ($is_add) {
            $cartNum = $cartInfo['cart_num']+1;
            if($stock < $cartInfo['cart_num']+1) return self::setErrorInfo('库存不足');
        } else {
            $cartNum = $cartInfo['cart_num']-1;
        }
    ...
    public static function getUserProductCartList($uid,$cartIds=[],$status=0)
    {
        ...
        if($cartIds) $model = $model->whereIn('id',$cartIds);
...
//                $invalid[] = $cart;
    ...
    *  获取购物车内最新一张产品图
    ...
    /**
     * 获取购物车list  的  商品总数量和总重量
     * @param array $cartInfoValid |购物车信息
     * @param float $defaultWeight | 单件默认重量(kg)
     * @return float|int
     */
    public static function getWeightFromCart(array $cartInfoValid ,float  $defaultWeight){

        $weight=0;

        foreach ($cartInfoValid as $validItem){
            $weightTemp= isset($validItem['productInfo']['weight'])?$validItem['productInfo']['weight']:$defaultWeight;
            $weight+= $validItem['cart_num']  * $weightTemp;
        }
        return $weight;
    }
