...
use common\repositories\PaymentRepositories;
...
use common\services\WechatTemplateService;
use common\services\workerman\ChannelService;
...
class StoreOrder extends BaseModel
{
    ...
    public function user(){
        return $this->hasOne("app\admin\model\user\User",'uid','uid');
    }
    ...
    protected static $payType = ['weixin_mp_pay' => '微信支付', 'yue' => '余额支付', 'weixinh5' => '微信H5支付', 'alipay' => '支付宝支付', 'offline' => '线下支付'];

    //订单状态（
    protected static $statusStrMap = [
        -1 => '申请退款',
        -2 => '退货成功',
        0 => '待发货',
        1 => '待收货',
    ...
    protected static $deliveryType = ['send' => '商家配送', 'express' => '快递配送','delivery'=>'即时配送'];

    public function getStatusStr($status)
    {
        $str=self::$statusStrMap[$status];
        if (!$str){
            $str='未知';
        }
        return $str;
     ...
     * @return array
     */
    public static function getOrderPriceGroup($cartInfo, $is_cart = 1)
    {
//        $storePostage = floatval(sys_config('store_postage')) ?: 0;//邮费基础价
        ...
        if ($is_cart) {
            ...
            $totalPrice = $cartInfo[0]['truePrice'];//获取订单总金额
        }
...
//        }
        ...
        return compact('storePostage', 'storeFreePostage', 'totalPrice');
    }
     ...
     * @throws \Psr\SimpleCache\InvalidArgumentException
     */
    public static function cacheKeyCreateOrder($uid, $key, $addressId, $payType, $useIntegral = false, $couponId = 0, $mark = '', $test = false, $shipping_type = 1, $take_time = 0)
    {
        self::beginTrans();
        try {
            ...
            if ($shipping_type === 2) {
                ...
                $addressInfo['real_name'] = $userAddress['real_name'];
            ...
            if ($useIntegral && $userInfo['integral'] > 0) {
                ...
                if ($deductionPrice < $payPrice) {
                    ...
                    $integral = bcsub($userInfo['integral'], $useIntegral, 2);
                    $SurplusIntegral = 0;
                    $res2 = false !== User::edit(['integral' => $integral], $userInfo['uid'], 'uid');
                } else {
            ...
            $orderDeliveryRes= ( new OrderDelivery())->createRow($order['order_id'], $storeId ,$addressInfo, $cartInfo);
            if (!$orderDeliveryRes) return self::setErrorInfo('配送订单生成失败!', true);
...
//        if($orderInfo['pay_type'] != 'yue') return self::setErrorInfo('该订单不能使用余额支付!');
     ...
     * @throws \think\exception\DbException
     */
    public static function aliPay($order_id, $uid, $formId = '')
    {
        $orderInfo = self::where('uid', $uid)->where('order_id', $order_id)->where('is_del', 0)->find();
        if (!$orderInfo) return self::setErrorInfo('订单不存在!');
        if ($orderInfo['paid']) return self::setErrorInfo('该订单已支付!');
//        if($orderInfo['pay_type'] != 'yue') return self::setErrorInfo('该订单不能使用余额支付!');
     ...
     * @return bool
     */
    public static function paySuccess($orderId, $paytype = 'weixin_mp_pay', $formId = '')
    {
        $order = self::where('order_id', $orderId)->find();
        $resPink = true;
        $res1 = self::where('order_id', $orderId)->update(['paid' => 1, 'status' => 0, 'pay_type' => $paytype, 'pay_time' => time()]);//订单改为支付
...
//        ]);
    ...
    public static function orderTakeAfter($order)
    {
        ...
        if ($order['is_channel'] == 1) {//小程序
            ...
            \common\services\WechatTemplateService::sendTemplate($openid, \common\services\WechatTemplateService::ORDER_TAKE_SUCCESS, [
                'first' => '亲，您的订单已收货',
    ...
    public static function removeOrder($uni, $uid)
    {
        ...
        if (!in_array($order['_status']['_type'],[0,-2,4]))
    ...
    public static function tidyOrder($order, $detail = false, $isPic = false)
    {
        ...
        $status = [];
        if (!$order['paid'] && $order['pay_type'] == 'offline') {
            $status['_type'] = 9;
            $status['_title'] = '线下付款';
            ...
            $status['_title'] = '未支付';
            //系统预设取消订单时间段
            $keyValue = ['order_cancel_time', 'order_activity_time'];
            //获取配置
        ...
        //获取产品状态图片
        if ($isPic) {
            $order_details_images = \common\services\GroupDataService::getData('order_details_images') ?: [];
...
//        $orderId = StorePink::where('uid',$uid)->where('status',1)->column('order_id','id');//获取正在拼团的订单编号
        ...
        if ('' === $status)
            ...
            return $model->where('paid', 0)->where('status', 4)->where('refund_status', 0);
            ...
            return $model->where('paid', 0)->where('status', -3)->where('refund_status', 0);
        else if($status == 9)//线下付款
            return $model->where('paid', 0)->where('pay_type', 'offline');
        else
            return $model;
    }
...
//    }
    ...
    public static function getUserOrderList($uid, $status = '', $page = 0, $limit = 8)
    {
        $list = self::statusByWhere($status, $uid)->where('is_del', 0)->where('uid', $uid)
            ->field('add_time,id,order_id,pay_price,total_num,total_price,pay_postage,total_postage,paid,status,refund_status,delivery_type,pay_type,coupon_price,deduction_price,is_del,shipping_type')
     ...
     * @throws \think\exception\DbException
     */
    public static function searchUserOrder($uid, $search)
    {
        $orders = self::alias('o')
            ->field('o.id,o.order_id,o.pay_price,o.add_time,o.total_num,o.total_price,o.pay_postage,o.total_postage,o.paid,o.status,o.refund_status,o.pay_type,o.coupon_price,o.deduction_price,o.delivery_type,o.shipping_type,o.is_del')
            ->field('p.store_name')
            ->join('store_order_cart_info oc','oc.oid=o.id')
            ->join('store_product p','p.id=oc.product_id')
            ->where('o.uid', $uid)
        ...
        if (!$orders) {
            ...
            foreach ($orders as $k => $order) {
                $orders[$k] = self::tidyOrder($order->toArray(), true);
            }
            return $orders;
        }
    }
...
//        $data['sum_price'] = self::where('is_del', 0)->where('paid', 1)->where('uid', $uid)->where('refund_status', 0)->sum('pay_price');
        //订单待支付 数量
        $data['unpaid_count'] = self::statusByWhere(4, $uid)->where('is_del', 0)->where('uid', $uid)->count();
        //订单待发货 数量
