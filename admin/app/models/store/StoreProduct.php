...
use app\models\store\StoreProductLabel;
...
//    public static function getValidProduct($productId, $field = 'cost,cate_id,description,ficti,id,image,is_benefit,is_best,is_hot,is_new,is_show,mer_id,ot_price,price,sales,slider_image,stock,store_info,store_name,unit_name,vip_price,IFNULL(sales,0) + IFNULL(ficti,0) as fsales')
//    {
//        $Product = self::where('is_del', 0)->where('is_show', 1)->where('id', $productId)->field($field)->find();
//        if ($Product) return $Product->toArray();
//        else return false;
//    }
    public static function getValidProduct($productId, $field = 'browse,cate_id,cost,description,ficti,give_integral,id,image,is_benefit,is_best,is_hot,is_new,is_seckill,is_show,keyword,mer_id,ot_price,price,sales,slider_image,sort,stock,store_info,store_name,unit_name,vip_price,IFNULL(sales,0) + IFNULL(ficti,0) as fsales')
    {
        $Product = self::where('is_del', 0)->where('is_show', 1)->where('id', $productId)->field($field)->find();
        if ($Product) return $Product->toArray();
        else return false;
    ...
    public static function getValidProducts($productId = '')
    {
        $ids = explode(',',$productId);
        return self::whereIn('id', $ids)->where('is_show', 1)->field('id,cate_id,price')->select()->toArray();
    }

    public static function getProduct($productId, $uniqueId)
    {
        $Product = self::alias('p')
            ->field('p.id,p.image,p.store_info,p.store_name,p.unit_name,p.give_integral,p.cate_id')
            ->field('pa.price,pa.stock')
            ->join('store_product_attr_value pa','pa.product_id=p.id')
            ->where('pa.unique', $uniqueId)
            ->where('p.is_del', 0)->where('p.is_show', 1)->where('p.id', $productId)->find();
        if ($Product) return $Product->toArray();
        else return [];
    }
    ...
    {
        return self::validWhere()->where('is_good', 1)->order('sort desc,id desc')->limit($limit)->field($field)->select()->toArray();
    }
...
//        if($salesOrder) $baseOrder = $salesOrder == 'desc' ? 'sales DESC' : 'sales ASC';//真实销量
        ...
        $list = $model->page((int)$page, (int)$limit)->field('id,store_name,keyword,cate_id,image,IFNULL(sales,0) + IFNULL(ficti,0) as sales,price,stock')->select()->each(function ($item) use ($uid, $type) {
            if ($type) {
        ...
        $product_labels = StoreProductLabel::getLabels($product_ids);
        foreach ($list as $k => $v) {
            $label = [];
            foreach ($product_labels as $key => $val) {
                if ($v['id'] == $val['product_id']) {
                    array_push($label,$val['label_name']);
                }
            }
            $list[$k]['label_names'] = $label;
        }
        return self::setLevelPrice($list, $uid);
    }

    public static function getDiscoveryList($data, $uid)
    {
        $sId = $data['sid'];
        $cId = $data['cid'];
        $keyword = $data['keyword'];
...
//        if($salesOrder) $baseOrder = $salesOrder == 'desc' ? 'sales DESC' : 'sales ASC';//真实销量
    ...
    public static function getConfirmOrderProduct($uid,$product_id=0,$uniqueId=0)
    {
        $productInfoField = 'id,cate_id,image,postage,give_integral,stock,store_name,unit_name,is_show,is_del,price';
        $model = new self();
        $valid = $invalid = $carts = [];
        $product = self::field($productInfoField)
            ->find($product_id)->toArray();
        $product['image'] = UtilService::setSiteUrl($product['image']);
        $carts['productInfo'] = $product;
        $carts['cart_num'] = 1;
        $carts['id'] = 0;
        //商品不存在
        if(!$product){
            $model->where('id',$carts['id'])->update(['is_del'=>1]);
            //商品删除或无库存
        }else if(!$product['is_show'] || $product['is_del'] || !$product['stock']){
            $invalid[] = $carts;
            //商品属性不对应
        }else if(!StoreProductAttr::issetProductUnique($product_id,$uniqueId)){
            $invalid[] = $carts;
            //正常商品
        }else{
            if($uniqueId){
                $attrInfo = StoreProductAttr::uniqueByAttrInfo($uniqueId);
                //商品没有对应的属性
                if(!$attrInfo || !$attrInfo['stock'])
                    $invalid[] = $carts;
                else{
                    $carts['productInfo']['attrInfo'] = $attrInfo;
                    $carts['truePrice'] = (float)self::setLevelPrice($attrInfo['price'],$uid,true);
                    $carts['vip_truePrice'] = (float)self::setLevelPrice($attrInfo['price'], $uid);
                    $carts['trueStock'] = $attrInfo['stock'];
                    $carts['productInfo']['image'] = empty($attrInfo['image']) ? $carts['productInfo']['image'] : $attrInfo['image'];
                    $valid[] = $carts;
                }
            }else{
                $carts['truePrice'] = (float)self::setLevelPrice($carts['productInfo']['price'],$uid,true);
                $carts['vip_truePrice'] = (float)self::setLevelPrice($carts['productInfo']['price'], $uid);
                $carts['trueStock'] = $carts['productInfo']['stock'];
                $valid[] = $carts;
            }
        }
