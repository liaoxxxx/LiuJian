...
class UserLevel extends BaseModel
{
    ...
    {
        return  self::where('uid', $uid)->order('grade desc')->field('grade,growth,level_id')->find();
    }
    ...
    public static function getUserLevel($uid,$grade=0)
    {
        ...
        $level = $model->where('uid', $uid)->order('grade desc')->field('level_id,is_forever,valid_time,id,status,grade,growth')->find();
    ...
    public static function getUserLevelInfo($id,$keyName=''){
        $vipinfo=self::valiWhere('a')->where('a.id',$id)->field('l.id,a.add_time,l.discount,a.level_id,a.valid_time,l.name,l.money,l.icon,l.is_pay,l.grade')
