                ...
                <div id="store-attr" class="mp-form" v-cloak="">
                    ...
                    <i-Form :label-width="80" style="width: 100%">
                        <template>
                            ...
                            <Alert type="success">总门店站点（用于处理超出配送送范围，地址信息，联系信息错误等的配送订单）</Alert>
                            <Form-Item>
                                <Row>
                                    <Select v-model="form.headStoreId" style="width:300px;height: 24px">
                                        <Option v-for="item in storeList" :value="item.id" :key="item.value">{{ item.name }}&nbsp;&nbsp;&nbsp;&nbsp; | &nbsp;&nbsp;&nbsp;&nbsp; {{ item.address }}</Option>
                                    </Select>
                                </Row>
                            </Form-Item>
                        </template>
        ...
        new Vue({
            data: function () {
                return {
                    ...
                    form: {
                        ...
                        headStoreId:storeDeliveryConfigData.headStoreId ,  //总店 ，总站点
            ...
            methods: {
                ...
                getStoreList: function () {
                    ...
                    let that = this;
                    $eb.axios.post('/admin/system.SystemStore/store_list', {}).then((res) => {
                        res= res.data
                        if (res.code === 0) {
                            console.log(9999999)
                ...
                submit: function () {
                    ...
                    $eb.axios.post('{:Url("")}' + (that.id ? '?id=' + that.id : ''), that.form).then(function (res) {
                        layer.close(index);
