...
use app\admin\model\store\SystemGroupDataProduct as GroupDataProductModel;
...
class StoreProduct extends AuthController
{
    ...
    {
        $group_data_id= $this->request->param('group_data_id');
        $type = $this->request->param('type');

        //活动专题商品id
        $active_product_ids = GroupDataProductModel::where('group_data_id',$group_data_id)->select()->toArray();
        $active_product_ids = $active_product_ids? array_column($active_product_ids,'product_id'):[];
        //获取分类
        $this->assign('cate',CategoryModel::getTierList(null, 1));
        //出售中产品
        $onsale =  ProductModel::where('is_del',0)->where('is_show',1)->count();
        ...
        $recycle =  ProductModel::where('is_del',1)->count();
        if($type == null) $type = 1;
        $this->assign(compact('group_data_id','active_product_ids','type','onsale','forsale','warehouse','outofstock','policeforce','recycle'));
        return $this->fetch();
    }
     ...
     * @return json
     */
    public function active_product_ist(){
        $where=Util::getMore([
            ['page',1],
            ['limit',20],
            ['store_name',''],
            ...
            ['order',''],
            ['type',$this->request->param('type')],
            ['active_product_ids',$this->request->param('active_product_ids')]
        ]);
...
//        return $this->fetch('public/common_form');
        $field = [
            ...
            Form::number('weight','单件重量',1.0)->min(0)->col(8),
    ...
    public function edit($id)
    {
        ...
        $field = [
            ...
            Form::number('weight','单件重量',$product->getData('weight'))->min(0)->col(8),
...
//        $result = StoreProductAttrResult::getResult($id);
    ...
    {
        if(!$id) return $this->failed('数据不存在!');
        $labels = StoreProductLabel::getStoreProductLabel($id);
        $result = [];
        if ($labels) {
            foreach ($labels as $key=>$value){
                $result[$key]['label_name'] = $value['label_name'];
                $result[$key]['attrHidden'] = true;
            }
        }
        $this->assign(compact('id','result'));
        return $this->fetch();
    }
    ...
    public function set_label($id)
    {
        if(!$id) return $this->failed('产品不存在!');
        list($attr) = Util::postMore([
            ['items',[]]
        ],null,true);
        $res = StoreProductLabel::createProductLabel($attr,$id);
        if($res)
            return $this->successful('编辑标签成功!');
        else
            return $this->failed(StoreProductLabel::getErrorInfo());
    }

    public function clear_label($id)
    {
        if(!$id) return $this->failed('产品不存在!');
        if(false !== StoreProductLabel::clearProductLabel($id))
            return $this->successful('清空产品标签成功!');
        else
            return $this->failed(StoreProductLabel::getErrorInfo('清空产品标签失败!'));
    }
