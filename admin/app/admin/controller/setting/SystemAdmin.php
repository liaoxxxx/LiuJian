...
use app\admin\controller\system\SystemStore;
...
class SystemAdmin extends AuthController
{
    ...
    {
        $list=AdminModel::where(self::$map)->select();
        return Json::successful('获取管理员列表成功!',['list'=>$list]);
    }
    ...
    public function create()
    {
        ...
        $f[] = Form::select('roles','绑定门店')->setOptions(function ()use($admin){
            $list = SystemRole::getRole(bcadd($admin->level,1,0));
            $options = [];
            foreach ($list as $id=>$roleName){
                $options[] = ['label'=>$roleName,'value'=>$id];
     ...
     *
     */
    public function getBindingStore()
    {
        $adminInfo= Session::get('adminInfo');

        //AdminModel::where('id',1)->update(['store_ids'=>json_encode([1,2,3,4])]);
        $storeList=  AdminModel::getBindingStore(json_decode($adminInfo['store_ids']));
        return Json::successful('获取门店成功!',['list'=>$storeList]);
    }

    public function bindStore(){
        if ($this->request->isPost()){
            $adminId= $this->request->post('adminId',null);
            $storeList=$this->request->post('storeList',null);
            $res=AdminModel::saveBindStore($adminId,$storeList);
            if ($res){
                return Json::successful('绑定门店成功!',['list'=>$storeList]);
            }
            return Json::fail('绑定门店 更新失败!',['list'=>$storeList]);
        }
        return $this->fetch();
    }
}
