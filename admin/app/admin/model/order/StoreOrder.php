...
use common\services\PHPExcelService;
...
class StoreOrder extends BaseModel
{
    ...
    public static function orderCount()
    {
        ...
        $data['cancel'] = self::statusByWhere(-5, new self())->where(['is_system_del' => 0])->count();
    ...
    public static function OrderList($where)
    {
        ...
        foreach ($data as &$item) {
            ...
            if ($item['paid'] == 0 && $item['status'] == 4) {
                ...
                $item['status_name'] = '已收货待评价';
            } else if ($item['paid'] == 1 && $item['status'] == 3 && $item['refund_status'] == 0) {
                $item['status_name'] = '已完成';
            } else if ($item['paid'] == 0 && $item['status'] == -3 && $item['refund_status'] == 0) {
...
HTML;
    ...
    public static function systemPage($where, $userid = false)
    {
        ...
        if ($where['export'] == 1) {
            ...
            foreach ($list as $index => $item) {
                ...
                if ($item['pay_type'] == 'weixin_mp_pay') {
                    $payType = '微信小程序支付';
                } elseif ($item['pay_type'] == 'alipay') {
                    $payType = '支付宝';
                } elseif ($item['pay_type'] == 'weixin') {
                    $payType = '微信支付';
                } elseif ($item['pay_type'] == 'weixinh5') {
                    $payType = '微信H5支付';
    ...
    public static function statusByWhere($status, $model = null, $alert = '')
    {
        ...
        if ('' === $status)
            ...
            return $model->where($alert . 'status', -3);
        else
            return $model;
    }
        ...
        if (isset($where['pay_type'])) {
            switch ($where['pay_type']) {
                ...
                case 3:
                    ...
                    break;
                case 4:
                    $model = $model->where($aler . 'pay_type', 'weixin_mp_pay');
                    break;
                case 5:
...
//        if($where['is_del'] != '' && $where['is_del'] != -1) $model = $model->where('is_del',$where['is_del']);
    ...
    public static function payTypeCount()
    {
        ...
        $where['pay_type'] = 4;
        $weixin_mp_pay = self::getOrderWhere($where, new self)->count();
        $where['pay_type'] = 5;
        $alipay = self::getOrderWhere($where, new self)->count();
        $where['pay_type'] = 6;
        $weixinh5 = self::getOrderWhere($where, new self)->count();
        return compact('weixin', 'yue', 'offline', 'weixin_mp_pay', 'alipay', 'weixinh5');
    }
}