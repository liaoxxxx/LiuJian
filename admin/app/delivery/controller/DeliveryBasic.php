...
class DeliveryBasic extends BaseController
{
    ...
    function __construct(App $app)
    {
        ...
        $headerToken= $this->request->header('token',0);
        if(!$headerToken){
            $headerToken=$this->request->post('token',0);
        }
        if(!$headerToken){
            echo json_encode(['msg'=>'请在header参数 ，或请求参数传入 token','status'=>400]);exit();
            //return JsonService::fail('请在header参数 ，或请求参数传入 token');
        }
        if ($headerToken) {
            $key = $this->jwtTool->getKey();
            try {
                $decodeRes = Jwt::decode($headerToken, $key, array('HS256'));
    ...
    protected $jwtTool=null;


    /**
     * @var |配送员基本信息
     */
    protected $deliveryMan=null;
