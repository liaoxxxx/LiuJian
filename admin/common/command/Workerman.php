...
use Channel\Client;
use Channel\Server;
use common\services\workerman\chat\ChatService;
use common\services\workerman\WorkermanService;
...
class Workerman extends Command
{
    ...
    protected function execute(Input $input, Output $output)
    {
        ...
        if(!$server || $server == 'delivery') {
            var_dump('chat');
            //创建 delivery 长连接服务
            $this->chatWorkerServer = new Worker($this->config['delivery']['protocol'] . '://' . $this->config['delivery']['ip'] . ':' . $this->config['delivery']['port']);
            $this->chatWorkerServer->count = $this->config['delivery']['serverCount'];
        }
    ...
    {
        var_dump($this-$this->workerServer);
        var_dump($this-$this->channelServer);
