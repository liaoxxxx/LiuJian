...
class Redis
{
    ...
    public function __construct(){
        ...
        $config=config("cache");
        $redisConf= $config['stores']['redis']??[];
        if (empty($redisConf)){
            app('json')->fail("redis`s  config is undefined in /config/cache.php ");
        }
        try{
            $redis = new \Redis();


            $result= $redis->connect($redisConf['host'], $redisConf['port']);
            if (!$result){
                die(json_encode(['msg'=>'redis auth failed']));
            }
            if (!empty($redisConf['password']) || $redisConf['password'] !='' ){
                $result = $redis->auth($redisConf['password']);
            }
            if (!$result){
                die(json_encode(['msg'=>'redis auth failed']));
            }
