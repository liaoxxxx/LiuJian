...
class JwtTool
{
     ...
     * @return array
     */
    public  function genPayload(int $expireTime,array $extendInfo=[],string $iss='',string $sub=''){
        return  [
            'iss' => $iss,
            'iat' => time(),
            'exp' => time() + $expireTime,
...
//自己使用测试begin
$payload_test = array('iss' => 'delivery', 'iat' => time(), 'exp' => time() + 7200, 'nbf' => time(), 'sub' => 'www.admin.com', 'jti' => md5(uniqid('JWT') . time()));;
$token_test = JwtTool::getToken($payload_test);
echo "<pre>";
echo $token_test;

//对token进行验证签名
$getPayload_test = JwtTool::verifyToken($token_test);
echo "<br><br>";
