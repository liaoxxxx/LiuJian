<!-- glance 店铺购物车单页模板 -->
<template name="glanceShopcart">
	<view class="glance-shop-cart" style="background-color: #F5F5F5;" @touchmove="scrollhoming" @touchend="scrollhomed">
		<!-- 空购物车加载 -->
		
		<view v-if="loading">
			<skeleton :row="20" animate :loading="loading" style="margin-top:24rpx;">
			</skeleton>
		</view>

		<!-- 购物车商品 -->
		<!-- <view v-for="(thiscart,index) in cart" :key="index" style="background-color: #FFFFFF;"> -->
		<!-- 按营销活动分组的商品 -->
		
		<template v-else-if="cartForValid.length > 0">
			<view v-for="(item,i) in cartForValid" :key="i">
				<scroll-view style="width: 100%;white-space: nowrap;background: #f5f5f5;" scroll-x="true" :scroll-left='scrollposition'
				 scroll-with-animation="true" v-if="item.id > -99">
					<view class="glance-shop-cart-scrollx-items" style="display: inline-block;width: 100%;background: #fff;">
						<view class="glance-shop-cart-scrollx-items-item">
							<!-- 勾选 -->
							<view style="width: 10%;height: 100%;background-color: #FFFFFF;">
								<!-- 勾选 -->
								<view class="glance-shop-cart-scrollx-items-item-sel" :class="[ ordercart.indexOf(item.id) != -1 ? 'glance-shop-cart-itemselected-img':'glance-shop-cart-itemunselected-img']"
								 @click="clickitemselected(item.id)"></view>
							</view>
							<!-- 图片 -->
							<view style="width: 30%;height: 100%;text-align:center;">
								<image :src="item.productInfo.image | checkImg" mode="scaleToFill" style="height: 75px;width: 75px;line-height: 80px;padding-top: 5px;"></image>
							</view>
							<!-- 描述 -->
							<view class="glance-shop-cart-items-item-des">
								<!-- 名称 -->
								<view class="sigle-line-text" style="font-size: 16px;height: 33.33%;text-align: left;" @click="clickitemhref(item.product_id)">{{ item.productInfo.store_name }}</view>
								<!-- 属性 -->
								<view class="sigle-line-text" style="font-size: 13px;height: 33.33%;text-align: left;color: #ADADAD;" @click="clickitemhref(item.product_id)">{{ item.attributes }}</view>
								<!-- 价格 & 数量-->
								<view class="glance-shop-cart-items-item-pq">
									<view class="sigle-line-text" style="font-size: 15px;text-align: left;width: 50%;">￥{{item.truePrice}}</view>
									<!-- 数量操作 -->
									<view class="glance-shop-cart-items-item-opt">
										<!-- 减数量 -->
										<view class="glance-shop-cart-items-item-opt-quantity-minus" :class="{'text-grey': item.cart_num==1, 'bg-grey': item.cart_num==1}"
										 @click="minusitem(item.id)">-</view>
										<!-- 数量 -->
										<view class="glance-shop-cart-items-item-opt-quantity">{{ item.cart_num }}</view>
										<!-- 加数量 -->
										<view class="glance-shop-cart-items-item-opt-quantity-plus" @click="plusitem(item.id)">+</view>
									</view>
								</view>
							</view>
						</view>
					</view>
			
					<!-- 删除 -->
					<view class="glance-shop-cart-del" @click="clickdel(item.id)">
						<view class="glance-shop-cart-del-img"></view>
					</view>
				</scroll-view>
			</view>
		</template>
		<view v-else style="width: 100%;height: 250px;">
			<view class="glance-shop-cart-nullcart"></view>
			<view style="height: 30px;width: 100%;font-size: 15px;line-height: 30px;text-align: center;">您的购物车为空，快去添加心爱的商品吧！</view>
		</view>
		<!-- <view style="height: 10px;background-color: #F5F5F5;"></view> -->
		<!-- </view> -->

		<!-- 金额合计 -->
		<view class="glance-shop-cart-order">
			<!-- 全选 -->
			<view style="width: 12%;">
				<!-- 勾选 -->
				<view class="glance-shop-cart-scrollx-items-item-sel" :class="[ isselectedall ? 'glance-shop-cart-itemselected-img':'glance-shop-cart-itemunselected-img']"
				 @click="clickitemselectedall"></view>
			</view>
			<!-- 总数量 -->
			<view class="glance-shop-cart-total-cnt">已选：( {{ cntitems }} )</view>
			<!-- 总金额 -->
			<view class="glance-shop-cart-total-amt"><text class="heji">合计:</text><text class="heji_2">￥</text>{{ totalamount }}</view>
			<!-- 立即下单 -->
			<view class="glance-shop-cart-create-order" hover-class="tap" @click="createorder">立即下单</view>
		</view>
	</view>
</template>

<script>
	import {
		vuexData
	} from '../../common/commonMixin.js'
	import skeleton from '@/components/xinyi-skeleton/skeleton.vue'
	export default {
		components: {
			skeleton
		},
		data() {
			return {
				// scroll position
				scrollposition: 0,
				// 实际项目中 购物车数据可从本地缓存中获取或从服务端获取
				cart: [],
				// 订单商品 购物车里已选择的商品生成订单
				ordercart: [],
				loading: true,
				// // 合计金额
				// totalamount: 0,
				// // 购物车商品数量 
				// cntitems: 0,
				// 显示空购物车背景
				shownullcart: false
			};
		},
		filters: {
			format(str) {
				let nowDay = moment().format('YYYY-MM-DD').split(' ')[0].split('-').slice(1)
				if (str) {
					let arr = str.split(' ')
					let date = arr[0]
					let dateArr = date.split('-').slice(1)
					if (nowDay[0] == dateArr[0] && nowDay[1] == dateArr[1]) {
						let timeStr = dateArr[0] + '月' + dateArr[1] + '日 [今天] ' + arr[1] + '前送达'
						return timeStr
					} else {
						let timeStr = dateArr[0] + '月' + dateArr[1] + '日 ' + arr[1] + '前送达'
						return timeStr
					}
				} else {
					return '-------'
				}
			},
			checkImg(str) {
				if (str) {
					let check = str.startsWith('http')
					if (check) {
						return str
					} else {
						return 'http://111.229.128.239:1003' + str
					}
				}
			}
		},
		mixins: [vuexData],
		async onLoad() {
			// 从缓存或服务端获取到购物车商品 这里示例数据如下：
			uni.showLoading({
				title: '努力加载中...'
			})
			await this.initCart()
			uni.hideLoading()
			this.loading = false
			// let objcart = [{
			// 		'items': this.cartForValid
			// 	},
			// ]

			// // 这里示例数据对象存入缓存
			// try {
			// 	uni.setStorageSync('cart', objcart);
			// } catch (e) {
			// 	// error
			// }
			// // 从缓存获取
			// try {
			// 	const objcart = uni.getStorageSync('cart');
			// 	if (objcart) {
			// 		// 装载this cart
			// 		this.cart = objcart;
			// 	}
			// } catch (e) {
			// 	// error
			// }

			// // console.log(this.cart)
			// // 默认勾选购物车所有商品 合计金额 合计数量
			// for (let i = 0; i < this.cart.length; i++) {
			// 	for (let k = 0; k < this.cart[i].items.length; k++) {
			// 		// 总金额 
			// 		this.totalamount = this.totalamount + this.cart[i].items[k].productInfo.price * this.cart[i].items[k].cart_num
			// 		// 总数量
			// 		this.cntitems = this.cntitems + this.cart[i].items[k].cart_num
			// 	}
			// }
			// this.ordercart = this.cartForValid
			// this.totalamount = this.fmamount(this.totalamount)
		},

		// 下拉刷新
		onPullDownRefresh() {
			uni.startPullDownRefresh(this.cart)
		},
		// 停止刷新
		onPullDownRefresh() {
			setTimeout(function() {
				uni.stopPullDownRefresh();
			}, 1000);

		},
		computed: {
			// 购物车数量
			cntitems() {
				let num = 0
				this.ordercart.forEach(item => {
					this.cartForValid.forEach(it => {
						if (item == it.id) {
							num += it.cart_num
						}
					})
				})
				return num
			},
			// 合计金额
			totalamount() {
				let num = 0
				this.ordercart.forEach(item => {
					this.cartForValid.forEach(it => {
						if (item == it.id) {
							num += it.cart_num * it.truePrice
						}
					})
				})
				return num
			},
			// 是否全选
			isselectedall() {
				if (this.cartForValid.length == 0) {
					return false
				}
				return this.ordercart.length == this.cartForValid.length
			}
		},
		// 关闭购物车前 存入缓存
		beforeDestroy: function() {
			// 购物车数据更新缓存
			try {
				// uni.setStorageSync('cart', this.cart);
			} catch (e) {
				// error
			}
		},
		methods: {
			async initCart() {
				let cartList = await this.$api.getCartList()
				this.getCartList(cartList)
			},
			// 减少商品数量
			async minusitem(id) {
				let cart = this.cartForValid.filter(item => item.id == id)[0]
				let num = cart.cart_num
				if (num == 1) {
					uni.showToast({
						title: '商品不可以减少了～～',
						icon: 'none'
					})
					return
				}
				let result = await this.$api.cartUpdate({
					id: id,
					is_add: 0
				})
				let flag = this.checkRes(result)
				if (flag) {
					this.changeCartNum({
						id,
						num: -1
					})
					for (let i = 0; i < 2; i++) {
						this.clickitemselected(id)
					}
					this.initCart()
				}
			},
			// 添加商品数量
			async plusitem(id) {
				let result = await this.$api.cartUpdate({
					id: id,
					is_add: 1
				})
				let flag = this.checkRes(result)
				if (flag) {
					this.changeCartNum({
						id,
						num: 1
					})
					for (let i = 0; i < 2; i++) {
						this.clickitemselected(id)
					}
					this.initCart()
				}
			},
			// scroll x 归位
			scrollhoming() {
				this.scrollposition = this.scrollposition - 1
			},
			// scroll x 归位后重置 scrollposition
			scrollhomed() {
				this.scrollposition = 0
			},
			// 勾选
			clickitemselected(id) {
				if (this.ordercart.indexOf(id) !== -1) {
					let tempOrder = this.ordercart
					tempOrder.splice(this.ordercart.indexOf(id), 1)
					this.ordercart = tempOrder
				} else {
					this.ordercart.push(id)
				}
			},
			// 全部勾选
			clickitemselectedall() {
				if (this.isselectedall) {
					this.ordercart = []
				} else {
					this.ordercart = []
					this.cartForValid.forEach(item => {
						this.ordercart.push(item.id)
					})
				}
			},
			// 点击删除
			async clickdel(id) {
				uni.showModal({
					title: '提示',
					content: '确定要删除购物车宝贝？',
					success: async (res) => {
						if (res.confirm) {
							let result = await this.$api.delCart({
								ids: [id]
							})
							let res = this.checkRes(result, '宝贝删除成功～～')
							if (res) {
								await this.initCart()
								this.scrollhoming()
							}
						} else if (res.cancel) {
							console.log('用户点击取消');
						}
					}
				})
			},
			// 点击商品href
			clickitemhref(id) {
				uni.navigateTo({
					url: `/pages/index/goods_info?id=${id}`
				})
			},
			// 	// 点击全选
			// 	clickitemselectedall() {
			// 		this.isselectedall = !this.isselectedall
			// 		// 全选
			// 		if (this.isselectedall) {
			// 			if (this.cart) {
			// 				for (var i = 0; i < this.cart.length; i++) {
			// 					for (let k = 0; k < this.cart[i].items.length; k++) {
			// 						// 未选的则选
			// 						if (this.cart[i].items[k].id < 0) {
			// 							this.cart[i].items[k].id = -this.cart[i].items[k].id
			// 							// 更新总数量
			// 							this.cntitems = this.cntitems + this.cart[i].items[k].cart_num
			// 							// 更新总金额
			// 							this.totalamount = this.totalamount + this.cart[i].items[k].productInfo.price * this.cart[i].items[k].cart_num
			// 							this.totalamount = this.fmamount(this.totalamount)
			// 						}
			// 					}
			// 				}
			// 				this.ordercart = this.ordercart.filter(item => true)
			// 			}
			// 		} else {
			// 			// 全不选
			// 			// this.cntitems = 0
			// 			if (this.cart) {
			// 				for (var i = 0; i < this.cart.length; i++) {
			// 					for (let k = 0; k < this.cart[i].items.length; k++) {
			// 						// 累计总金额和总数量 勾选时加
			// 						if (this.isselected(this.cart[i].items[k].id)) {
			// 							this.cart[i].items[k].id = -this.cart[i].items[k].id
			// 							// 更新总数量
			// 							this.cntitems = this.cntitems - this.cart[i].items[k].cart_num
			// 							// 更新总金额
			// 							this.totalamount = this.totalamount - this.cart[i].items[k].productInfo.price * this.cart[i].items[k].cart_num
			// 							this.totalamount = this.fmamount(this.totalamount)
			// 						}
			// 					}
			// 				}
			// 				this.ordercart = this.ordercart.filter(item => false)
			// 			}
			// 		}
			// 	},
			// 	// 点击勾选
			// 	clickitemselected(id) {
			// 		if (this.cart) {
			// 			for (var i = 0; i < this.cart.length; i++) {
			// 				for (let k = 0; k < this.cart[i].items.length; k++) {
			// 					if (this.cart[i].items[k].id == id) {
			// 						this.cart[i].items[k].id = -this.cart[i].items[k].id
			// 						// 累计总金额和总数量 勾选时加
			// 						if (this.isselected(this.cart[i].items[k].id)) {
			// 							console.log(this.cart[0].items.filter(item => item.id == id), this.cart[0].items, id)
			// 							this.ordercart.push(this.cart[0].items.filter(item => item.id == -id)[0])
			// 							// 更新总数量
			// 							this.cntitems = this.cntitems + this.cart[i].items[k].cart_num
			// 							// 更新总金额
			// 							this.totalamount = this.totalamount + this.cart[i].items[k].productInfo.price * this.cart[i].items[k].cart_num
			// 							this.totalamount = this.fmamount(this.totalamount)
			// 							// 最后已勾选则 全选
			// 							if (this._isselectedall()) {
			// 								this.isselectedall = true
			// 							}
			// 						} else {
			// 							// 取消勾选时减
			// 							this.ordercart = this.ordercart.filter(item => item.id != id)
			// 							// 更新总数量
			// 							this.cntitems = this.cntitems - this.cart[i].items[k].cart_num
			// 							// 更新总金额
			// 							this.totalamount = this.totalamount - this.cart[i].items[k].productInfo.price * this.cart[i].items[k].cart_num
			// 							this.totalamount = this.fmamount(this.totalamount)
			// 							this.isselectedall = false
			// 						}
			// 						return
			// 					}
			// 				}
			// 			}
			// 		}
			// 	},
			// 点击删除
			// async clickdel(itemid) {
			// 	uni.showModal({
			// 		title: '提示',
			// 		content: '确定要删除购物车宝贝？',
			// 		success: async (res) => {
			// 		        if (res.confirm) {
			// 		            let result = await this.$api.delCart({ids: [itemid]})
			// 					if (result.status == 200) {
			// 						for (let i = 0; i < this.cart.length; i++) {
			// 							for (let k = 0; k < this.cart[i].items.length; k++) {
			// 								if (this.cart[i].items[k].id == itemid) {
			// 									// 勾选状态下更新数量和金额
			// 									if (this.isselected(this.cart[i].items[k].id)) {
			// 										// 更新总数量
			// 										this.cntitems = this.cntitems - this.cart[i].items[k].cart_num
			// 										// 更新总金额
			// 										this.totalamount = this.totalamount - this.cart[i].items[k].productInfo.price * this.cart[i].items[k].cart_num
			// 										this.totalamount = this.fmamount(this.totalamount)
			// 										// 删除商品
			// 										this.cart[i].items[k].id = -99 - itemid
			// 									} else {
			// 										// 未勾选时 删除商品 列表中不再显示 id= -99-原始id
			// 										this.cart[i].items[k].id = -99 + itemid
			// 									}
			// 									// 如果全部删除 则显示空购物车背景
			// 									if (this._isdeledall()) {
			// 										this.shownullcart = true
			// 										this.isselectedall = false
			// 									}
			// 									return
			// 								}
			// 							}
			// 						}
			// 					}
			// 		        } else if (res.cancel) {
			// 		            console.log('用户点击取消');
			// 		        }
			// 		    }
			// 	})

			// 	// 删除购物车商品时 更新合计金额

			// },
			// 	// 减数量
			// 	async minusitem(itemid) {
			// 		for (let i = 0; i < this.cart.length; i++) {
			// 			for (let k = 0; k < this.cart[i].items.length; k++) {
			// 				if ((this.cart[i].items[k].id == itemid) && (this.cart[i].items[k].cart_num > 0)) {
			// 					// 更新item数量
			// 					this.cart[i].items[k].cart_num = this.cart[i].items[k].cart_num - 1
			// 					// 勾选状态下更新数量和金额
			// 					if (this.isselected(this.cart[i].items[k].id)) {
			// 						// 更新总数量
			// 						this.updatecntitems(-1)
			// 						// 更新总金额
			// 						this.updatetotalamt(-this.cart[i].items[k].productInfo.price)
			// 					}
			// 					// 数量减为0时 不勾选
			// 					if (this.cart[i].items[k].cart_num == 0) {
			// 						this.cart[i].items[k].id = this._unselected(this.cart[i].items[k].id)
			// 					}
			// 					let result = await this.$api.cartUpdate({id: itemid, is_add: 0})
			// 					return
			// 				}
			// 			}
			// 		}
			// 	},
			// 	// 加数量
			// 	async plusitem(itemid) {
			// 		for (let i = 0; i < this.cart.length; i++) {
			// 			for (let k = 0; k < this.cart[i].items.length; k++) {
			// 				// 这里需要进行超卖控制 商品可售卖的数量 这里面示例可售卖100
			// 				if ((this.cart[i].items[k].id == itemid) && (this.cart[i].items[k].cart_num < 100)) {
			// 					// 更新item数量
			// 					this.cart[i].items[k].cart_num = this.cart[i].items[k].cart_num + 1
			// 					// 勾选状态下更新数量和金额
			// 					if (this.isselected(this.cart[i].items[k].id)) {
			// 						// 更新总数量
			// 						this.updatecntitems(1)
			// 						// 更新总金额
			// 						this.updatetotalamt(this.cart[i].items[k].productInfo.price)
			// 					} else {
			// 						// 加数量时未勾选则 勾选
			// 						this.cart[i].items[k].id = this._selected(this.cart[i].items[k].id)
			// 						// 更新总数量、
			// 						this.cntitems = this.cntitems + this.cart[i].items[k].cart_num
			// 						// 更新总金额
			// 						this.totalamount = this.totalamount + this.cart[i].items[k].productInfo.price * this.cart[i].items[k].cart_num
			// 						this.totalamount = this.fmamount(this.totalamount)
			// 					}
			// 					let result = await this.$api.cartUpdate({id: itemid, is_add: 1})
			// 					console.log(result)
			// 					// 最后已勾选则 全选
			// 					if (this._isselectedall()) {
			// 						this.isselectedall = true
			// 					}
			// 					return
			// 				}
			// 			}
			// 		}
			// 	},


			// 	// 更新合计金额
			// 	updatetotalamt(amt) {
			// 		amt = Number(amt)
			// 		this.totalamount = this.totalamount + amt
			// 		this.totalamount = this.fmamount(this.totalamount)

			// 	},
			// 	// 更新合计数量
			// 	updatecntitems(cnt) {
			// 		this.cntitems = this.cntitems + cnt
			// 	},
			// 	// 格式化金额
			// 	fmamount(amt) {
			// 		return Math.round(amt * 100) / 100
			// 	},
			// 	// 不勾选
			// 	_unselected(id) {
			// 		return -Math.abs(id)
			// 	},
			// 	// 勾选
			// 	_selected(id) {
			// 		return Math.abs(id)
			// 	},
			// 	// 是否勾选
			// 	isselected(id) {
			// 		return id > 0
			// 	},
			// 	// 是否全部已勾选
			// 	_isselectedall() {
			// 		for (let i = 0; i < this.cart.length; i++) {
			// 			for (let k = 0; k < this.cart[i].items.length; k++) {
			// 				// 存在一个未勾选 则未全选
			// 				if (this.cart[i].items[k].id < 0) {
			// 					return false
			// 				}
			// 			}
			// 		}
			// 		return true
			// 	},
			// 	// 是否全部删除
			// 	_isdeledall() {
			// 		for (let i = 0; i < this.cart.length; i++) {
			// 			for (let k = 0; k < this.cart[i].items.length; k++) {
			// 				// 存在 未删除
			// 				if (this.cart[i].items[k].id > -99) {
			// 					return false
			// 				}
			// 			}
			// 		}
			// 		return true
			// 	},
			// 	// 生成订单
			createorder() {
				// 合计金额大于0 创建订单
				if (this.ordercart.length == 0) {
					uni.showModal({
						content: '请选择下单的商品！'
					})
				} else {
					let cartIds = this.ordercart.join(',')
					uni.navigateTo({
						url: '/pages/order/addOrder?cartIds=' + cartIds
					})
				}
			}
		}
	}
</script>

<style lang="scss">
	page {
		background: #f5f5f5;
		padding-bottom: 100rpx;
	}

	// 单行文本样式
	.sigle-line-text {
		width: 100%;
		overflow: hidden;
		white-space: nowrap;
		text-overflow: ellipsis;
		text-align: left;
	}

	.glance-shop-cart {
		width: 100%;
		// font-family: Arial, Helvetica, sans-serif;
	}

	// 滑动item 样式
	.glance-shop-cart-scrollx-items {
		width: 100%;
		height: 90px;
		display: flex;
		display: -webkit-flex;
		flex-flow: row nowrap;
		align-items: flex-start;
		justify-content: space-between;
		// padding: 10px 0 10px 0;
		// margin-top: 10px;
		padding-top: 15rpx;
	}

	// 滑动区域内item 样式
	.glance-shop-cart-scrollx-items-item {
		display: flex;
		display: -webkit-flex;
		flex-flow: row nowrap;
		align-items: flex-start;
		justify-content: flex-start;
		width: 100%;
		height: 100%;
	}

	// item 勾选样式
	.glance-shop-cart-scrollx-items-item-sel {
		position: relative;
		top: 30%;
		left: 60%;
		;
	}

	// item 描述信息样式
	.glance-shop-cart-items-item-des {
		width: 60%;
		height: 100%;
		display: -webkit-flex;
		flex-flow: column nowrap;
		align-items: flex-start;
		justify-content: flex-start;
	}

	// 价格和数量样式
	.glance-shop-cart-items-item-pq {
		width: 100%;
		height: 33.33%;
		display: flex;
		display: -webkit-flex;
		flex-flow: row nowrap;
		align-items: flex-start;
		justify-content: space-between;
	}

	// 数量操作
	.glance-shop-cart-items-item-opt {
		display: flex;
		display: -webkit-flex;
		flex-flow: row nowrap;
		align-items: center;
		justify-content: space-between;
		width: 50%;
		height: 100%;
		font-size: 20px;
		margin-right: 10px;
		text-align: center;
		color: #6C6C6C;
		line-height: 100%;

		// 减数量
		&-quantity-minus {
			border-style: solid;
			border-width: 1px 0px 1px 1px;
			border-color: #E0E0E0;
			width: 33.33%;
			height: 80%;
			position: relative;
			top: 25%;
			transform: translateY(-50%)
		}

		// 数量
		&-quantity {
			border-style: solid;
			border-width: 1px 1px 1px 1px;
			border-color: #E0E0E0;
			width: 33.33%;
			height: 80%;
			font-size: 13px;
			position: relative;
			top: 25%;
			transform: translateY(-50%);
		}

		// 加数量
		&-quantity-plus {
			border-style: solid;
			border-width: 1px 1px 1px 0;
			border-color: #E0E0E0;
			width: 33.33%;
			height: 80%;
			position: relative;
			top: 25%;
			transform: translateY(-50%);
		}
	}

	// 底部汇总和订单样式
	.glance-shop-cart-order {
		width: 100%;
		height: 60px;
		background-color: #ffffff;
		display: flex;
		display: -webkit-flex;
		flex-flow: row nowrap;
		align-items: center;
		justify-content: space-between;
		position: fixed;
		bottom: 0px;
	}

	// 合计金额样式
	.glance-shop-cart-total-cnt {
		width: 30%;
		text-align: left;
		line-height: 40px;
		font-size: 13px;
		margin-left: 15px;
	}

	// 合计金额样式
	.glance-shop-cart-total-amt {
		width: 35%;
		text-align: right;
		margin-right: 15px;
		line-height: 40px;
		font-size: 32rpx;
		margin-left: 10px;
		font-weight: bold;
		color: #f40;
		// margin-top: -30rpx;
	}

	.heji {
		font-size: 28rpx;
		color: #000;
		font-weight: bold;
		padding-right: 10rpx;
	}

	.heji_2 {
		font-size: 20rpx;
		font-weight: bold;
		color: #f40;
	}

	// 生成订单样式
	.glance-shop-cart-create-order {
		width: 30%;
		background-color: #f63b0d;
		text-align: center;
		line-height: 36px;
		font-size: 0.8rem;
		color: #ffffff;
		margin-right: 10px;
		border-radius: 50rpx;
	}

	// item 选择样式
	// 不勾选
	.glance-shop-cart-itemunselected-img {
		width: 16px;
		height: 16px;
		border: solid 1px lightgray;
		border-radius: 50%;
	}

	// 勾选
	.glance-shop-cart-itemselected-img {
		width: 20px;
		height: 20px;
		background-size: 20px 20px;
		background-repeat: no-repeat;
		background-image: url(data:image/png;base64,)
	}

	.glance-shop-cart-del {
		display: inline-block;
		width: 18%;
		height: 90px;
		background-color: red;
	}

	// 删除商品背景样式
	.glance-shop-cart-del-img {
		width: 30px;
		height: 30px;
		background-repeat: no-repeat;
		position: relative;
		top: 25%;
		transform: translateY(-50%);
		left: 50%;
		transform: translateX(-50%);
		background-image: url(data:image/png;base64,)
	}

	// 空购物车背景样式
	.glance-shop-cart-nullcart {
		width: 120px;
		height: 120px;
		position: relative;
		transform: translateY(-50%);
		left: 50%;
		transform: translateX(-50%);
		background-repeat: no-repeat;
		background-image: url(data:image/png;base64,)
	}

	.tap {
		opacity: .3;
	}
</style>
