...
function drawPolyline(origin, destination, color, width, callback) {
	return {
		...
		success(data) {
			...
			let distance = ((data.paths[0].distance) / 1000).toFixed(2);
			callback([{
				points: points,
				color: color,
				width,
...
// 获取路线 和距离
function drawLine(origin, destination, color, width, callback) {
	var myAmapFun = new amapFile.AMapWX({
		key: key
	});
	myAmapFun.getDrivingRoute(drawPolyline(origin, destination, color, width, callback))
}

// 获取地址描述
function getRegeo(callback, location) {
...
function getRegeoForKeyword(callback, keyword, location) {
	// location: '' + longitude + ',' + latitude + '',
	var myAmapFun = new amapFile.AMapWX({
		key: key
	});
